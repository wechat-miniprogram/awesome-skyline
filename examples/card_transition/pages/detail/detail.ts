...
const { shared, timing, Easing, snapshotContext } = wx.worklet

console.info('@@@@ snapshotContext ', snapshotContext)
...
Component({
  ...
  lifetimes: {
    created() {
      ...
      this.snapshotRef = shared(undefined)
      this._openDragSnapshot = shared(true)
  ...
  methods: {
    getSnapshotRef() {
      this.createSelectorQuery().select("#snapshot")
        .node().exec(res => {
          const node = res[0].node
          this.snapshotRef.value = node?.getNodeRef?.()
          console.info('@@@ getSnapshotRef ', this.snapshotRef.value)
        })
    },

    paintSnapshot() {
      'worklet'
      if (!this._openDragSnapshot.value) return
      if (!this.snapshotRef.value) return
      snapshotContext?.changeRenderMode(this.snapshotRef.value, 'snapshot')
    },
    ...
    paintView() {
      ...
      if (!this._openDragSnapshot.value) return
      if (!this.snapshotRef.value) return
      snapshotContext?.changeRenderMode(this.snapshotRef.value, 'view')
    },

    doSnapshotAnimation(state) {
      'worklet'
    ...
    handlePanGesture(e) {
      ...
      this.doSnapshotAnimation(e.state)
